...
MODELNAME = "WB_CLASSIFER"

if not os.path.exists("dataWBClassifier.pkl"):
    datasetLocal = {"train": [], "validation": [], "test": []}

    testFile = "./wbPerps.csv"
    ...
    pkl.dump(datasetLocal, open("dataWBClassifier.pkl", "wb"))
else:
    print("Loading from pickle")
    datasetLocal = pkl.load(open("dataWBClassifier.pkl", "rb"))
...
def testModel(model, testDataset, test=True):
    ...
    if test:
        ...
        cm.plot()
        # save
        plt.savefig("confusionWBClassifier.png")
        # clear plt
        plt.clf()
        ...
        plt.savefig("rocWBClassifer.png")
